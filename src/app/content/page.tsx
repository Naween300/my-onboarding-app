...
export default function ContentPage() {
  ...
  // Check LinkedIn and Twitter connection status on mount
  useEffect(() => {
    const checkConnections = async () => {
      try {
        const res = await fetch('/api/social-platforms/status');
        const data = await res.json();
        const linkedin = data.platforms?.find((p: any) => p.platform === 'linkedin');
        setLinkedinConnected(!!linkedin?.connected);
        const twitter = data.platforms?.find((p: any) => p.platform === 'twitter');
        setTwitterConnected(!!twitter?.connected);
      } catch (e) {
  ...
  const handlePostToLinkedIn = async () => {
    try {
      ...
      if (result.success) {
        ...
        alert('❌ LinkedIn post failed: ' + (result.error || 'Unknown error'));
      }
    } catch (error: any) {
      alert('❌ LinkedIn post error: ' + error.message);
    }
  };

  // Handler for posting to Twitter
  const handlePostToTwitter = async () => {
    try {
      const mediaUrl = 'https://images.unsplash.com/photo-1743376272672-c130603a3af2?q=80&w=1229&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D';
      ...
      if (result.success) {
        ...
        alert('❌ Twitter post failed: ' + (result.error || 'Unknown error'));
      }
    } catch (error: any) {
      alert('❌ Twitter post error: ' + error.message);
    }
  };
            ...
            <div className="flex flex-col space-y-2">
              <div className="border-t pt-2">
                <button 
                  ...
                  className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 font-medium mr-2"
                >
                  Facebook (Ayrshare)
                </button>
                <button 
                  onClick={handleInstagramPost}
                  className="bg-gradient-to-r from-pink-500 to-yellow-500 text-white px-6 py-2 rounded-lg hover:from-pink-600 hover:to-yellow-600 font-medium"
                >
                  Instagram (Graph API)
                </button>
                <button
                  ...
                  className={`bg-blue-700 text-white px-6 py-2 rounded-lg font-medium hover:bg-blue-800 disabled:opacity-50 disabled:cursor-not-allowed mt-2`}
                >
                  Post to LinkedIn
                </button>
                <button
                  ...
                  className={`bg-gray-800 text-white px-6 py-2 rounded-lg font-medium hover:bg-gray-900 disabled:opacity-50 disabled:cursor-not-allowed mt-2`}
                >
                  Post to Twitter
                </button>
              </div>
